...
begin
  ...
  LOffset := APagina * ATamanho;

  Result := TObjectList<TPessoaDTO>.Create(True);
  LPessoaList := FPessoaRepo.Listar(FConn, LOffset, ATamanho);
  try
    for LPessoa in LPessoaList do
      Result.Add(TPessoaDTO.FromEntity(LPessoa));
  finally
    LPessoaList.Free;
  end;
end;
...
begin
  ...
  FConn.StartTransaction;
  try
    LEnt := APessoa.ToEntity;
    try
      FPessoaRepo.Atualizar(FConn, LEnt);
    finally
      LEnt.Free;
    end;
    FConn.Commit;
