  ...
  TTask.Run(
    ...
    begin
      ...
      try
        ...
        on E: Exception do
        begin
          SetStatus('ViaCEP: erro inesperado - ' + E.Message);
          if TransacaoAberta and Conn.InTransaction then
            Conn.Rollback;
        end;
      end;
...
var
  LQuery: TFDQuery;
  LIdEnd: Int64;
  LDsCep, LCep8, LUrl, LJson: string;
  LResp: IHTTPResponse;
  LUf, LCidade, LBairro, LLogr, LCompl: string;
  LHttpCode: Integer;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        try
          LIdEnd := LQuery.FieldByName('idendereco').AsLargeInt;
          LDsCep := LQuery.FieldByName('dscep').AsString;
          LCep8 := StringReplace(LDsCep, '-', '', [rfReplaceAll]);

          if Length(LCep8) <> 8 then
          begin
            SetStatus(Format('CEP inválido: %s (IdEndereco=%d)', [LCep8, LIdEnd]));
            Inc(FFalhasSeguidas);
            LQuery.Next;
            Continue;
          end;

          LUrl := MontarUrlViaCep(LCep8);
          AguardaProximaRequisicao;
          LResp := AHttp.Get(LUrl);
          LHttpCode := LResp.StatusCode;

          if LHttpCode = 200 then
          begin
            FFalhasSeguidas := 0;
            LJson := LResp.ContentAsString(TEncoding.UTF8);
            if TentarLerViaCepJson(LJson, LUf, LCidade, LBairro, LLogr, LCompl) then
            begin
              AEnderecoRepo.GravarIntegracao(AConn, LIdEnd, LUf, LCidade, LBairro, LLogr, LCompl);
              Inc(AProcessados);
              Inc(LContador);
        ...
        if (LContador > 0) and (LContador mod C_COMMIT_A_CADA_REGISTROS = 0) then
        begin
          try
            if AConn.Connected and AConn.InTransaction then
              AConn.Commit;
