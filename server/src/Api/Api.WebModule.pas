...
uses Api.ServerMethods, Web.WebReq;

procedure TWebModule1.DSServerClass1GetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
  PersistentClass := Api.ServerMethods.TServerMethods1;
end;
...
begin
  if SameText(TagString, 'urlpath') then
    ...
    ReplaceText := Api.ServerMethods.TServerMethods1.ClassName
...
var
  ...
  LLote   : Integer;
  LBody   : TStringStream;
  LServer : TServerMethods1;
  LInser  : Integer;
  LBytes  : TBytes;
  LRead   : Integer;
begin
  LPath := LowerCase(string(Request.PathInfo));

  if LPath.StartsWith('/upload/pessoa_csv') then
  begin
    Handled := True;

    LParts := LPath.Split(['/'], TStringSplitOptions.ExcludeEmpty);
    if Length(LParts) >= 3 then
      LLote := StrToIntDef(LParts[2], 1000)
    else
      LLote := 1000;
    ...
    try
      ...
      begin
        SetLength(LBytes, Request.ContentLength);
        LRead := Request.ReadClient(LBytes[0], Length(LBytes));
        if LRead > 0 then
          LBody.WriteString(TEncoding.UTF8.GetString(LBytes, 0, LRead));
      end
      else
      begin
        LBody.WriteString(Request.Content);
      end;

      LServer := TServerMethods1.Create(nil);
      try
        LInser := LServer.Pessoa_ImportarCSV_Stream(LLote, LBody);
      finally
        LServer.Free;
      end;
      ...
      Response.Content     := Format('{"result":[%d]}', [LInser]);
    finally
      LBody.Free;
    end;
    Exit;
