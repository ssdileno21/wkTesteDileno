...
begin
  Result := Format('{"error": "%s"}', [AMsg.Replace('"', '\"')]);
end;
...
begin
  try
    ...
    FEnderecoService.DispararAtualizacaoViaCEP;
  except
    on E: Exception do
      raise Exception.Create('Erro ao disparar CEP: ' + E.Message);
  end;
end;
...
begin
  EnsureEnderecoService;
  Result := FEnderecoService.ViaCepTotalProcessados;
end;
...
begin
  try
    EnsureEnderecoService;
    Result := FEnderecoService.StatusAtualizacaoViaCEP;
  except
    on E: Exception do
      Result := JsonError('Não foi possível obter o status ViaCEP: ' + E.Message);
  end;
end;
...
begin
  EnsureEnderecoService;
  Result := FEnderecoService.ViaCepTotalEnderecos;
end;
...
begin
  try
    ...
    on E: Exception do
      raise Exception.Create('Erro em DispararCepEstaEmExecucao: ' + E.Message);
  end;
end;
